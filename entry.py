...
from utils import sheets, xp as xp_utils
...
def _step3():
    ...
    if "finance" in active_domains:
        with st.expander("💰 Personal Finance", expanded=True):
            c1, c2 = st.columns(2)
            with c1:
                savings_rate = st.number_input("Savings Rate %", min_value=0.0, max_value=100.0, format="%.1f", key="fin_sr")
            with c2:
                net_worth = st.number_input("Net Worth ($)", min_value=0.0, format="%.2f", key="fin_nw")
    ...
    with col_back:
        if st.button("← Back"):
            ...
            st.rerun()
    with col_submit:
        if st.button("✅ Submit Entry", use_container_width=True, type="primary"):
            _submit(domain_data)
