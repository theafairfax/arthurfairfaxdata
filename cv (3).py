...
def load_cv() -> pd.DataFrame:
    ...
    # Parse date — handles M/D/YYYY format (e.g. 9/27/2024, 12/8/2024)
    if "Date Acquired" in df.columns:
        df["Date Acquired"] = df["Date Acquired"].astype(str).str.strip()
        df["Date Sort"] = pd.to_datetime(
            df["Date Acquired"], format="%m/%d/%Y", errors="coerce"
        ).fillna(
            pd.to_datetime(df["Date Acquired"], errors="coerce")  # fallback: infer format
        )
